...
// Promise.all(promises).then(val => val.forEach(val => console.log(val)));

// - Create a list of 5 Github usernames in an array and using `Promise.all` get access to the data of each user from GitHub API. Log the number of followers of each user.

let username = ["anish979828","aditya2z","prank7","ritesh22201"];

let userpromises = username.map(user => fetch(`https://api.github.com/users/${user}`).then(val => val.json()));

Promise.all(userpromises).then(val => val.forEach(user => console.log(user.followers))); 
